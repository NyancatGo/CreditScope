...
SNAPSHOT_PATH = ROOT_DIR / "data" / "final_validation_snapshot.json"
...
def run_api_smoke_test() -> dict[str, Any]:
    ...
    checks = [
        ("index", "GET", "/", None),
        ("overview", "GET", "/genel-bakis", None),
        ("demo_scenarios", "GET", "/demo-senaryolari", None),
        ("model_monitoring", "GET", "/model-izleme", None),
        ("rules", "GET", "/kurallar", None),
...
def scenario_snapshot_key(scenario_id: str) -> str:
    return {
        "low_risk": "safe",
        "manual_review": "review",
        "high_risk": "risky",
        "edge_case_strong_score_weak_employment": "edge",
    }.get(scenario_id, scenario_id)


def write_validation_snapshot(metrics_payload: dict[str, Any], demo_predictions: list[dict[str, Any]]) -> None:
    metrics = metrics_payload["metrics"]
    comparison_df = pd.read_csv(OUTPUT_DIR / "model_comparison_metrics.csv")
    counts = load_json(ROOT_DIR / "outputs" / "shap" / "shap_error_analysis_summary.json")["group_analysis"]["counts"]
    ...
    for row in demo_predictions:
        ...
        scenarios.append(
            {
                "key": scenario_snapshot_key(row["id"]),
                "title": row["title"],
                "expected": row["expected_decision"],
                "actual": row["actual_decision"],
                "model_score": response["model_temerrut_olasiligi"],
                "adjusted_score": response["temerrut_olasiligi"],
                "dti": response["hesaplanan_dti"],
                "profile": {
                    "Gelir": format_money(summary["Income"]),
                    "Kredi Tutarı": format_money(summary["LoanAmount"]),
                    "Kredi Notu": summary["CreditScore"],
                    "İstihdam": summary["EmploymentType"],
                    "Çalışma Süresi": f"{summary['MonthsEmployed']} ay",
                    "Kefil": "Var" if summary["HasCoSigner"] == "Yes" else "Yok",
                },
                "applied_rules": [rule["id"] for rule in response.get("business_rule_adjustments", [])],
                "comment": row["comment"],
    ...
    snapshot = {
        "snapshot_version": "week13-final-validation-2026-04-20",
        "source": "Generated by tools/week10_demo_eval.py from final validation artifacts",
        "final_metrics": {
            "accuracy": round(float(metrics["accuracy"]), 4),
            "precision": round(float(metrics["precision"]), 4),
            "recall": round(float(metrics["recall"]), 4),
            "f1": round(float(metrics["f1"]), 4),
            "threshold": round(float(metrics_payload["decision_threshold"]), 3),
            "recall_target": float(metrics_payload["target_recall"]),
            "recall_floor": 0.67,
        },
        "model_comparison": [
            {
                "name": row.model if row.model != "XGBoost" else "XGBoost (Final)",
...
def main() -> None:
    ...
    try:
        ...
        write_validation_snapshot(metrics_payload, demo_predictions)
    finally:
